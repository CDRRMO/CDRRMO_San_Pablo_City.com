    ...
    $(function () {
    	...
    	$(document).ready(function() { 
			...
			$(document).on('submit', '.add-admin-form', function(event) {
		   	...
		   	$(document).on('submit', '.update-brgy-form', function(event) {
		    	event.preventDefault();
		    	var brgy_name = $('.brgy-name').val();
		    	var brgy_chairman = $('.brgy-chairman').val();
		    	var brgy_hotline = $('.brgy-hotline').val();
		    	var brgy_availability = $('.brgy-availability').val();
	    		if(brgy_name == ''){
	    			$('.modal-message').html('brgy. name cannot be empty');
	    		}
	    		else if(brgy_chairman == ''){
	    		...
	    		else if(brgy_hotline == ''){
	    			$('.modal-message').html('hotline number cannot be empty');
	    		}
	    		else if(brgy_availability == ''){
	    			$('.modal-message').html('available slot cannot be empty');
	    		}
	    		else{
	    			jQuery.ajax({
			            type: "POST",
			            url: "sql/update-brgy.php",
			            data: $(".update-brgy-form").serialize(),
			            dataType: "JSON",
			            cache: false,
			            success: function(strMessage) {
			            	if(strMessage.msg == '1'){
